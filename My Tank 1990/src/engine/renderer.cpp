...
void Renderer::clear() {
    ...
    SDL_RenderClear(m_renderer); // Limpa o buffer traseiro
}

void Renderer::flush() {
    SDL_RenderPresent(m_renderer); // Troca os buffers
}

void Renderer::drawObject(const SDL_Rect *texture_src, const SDL_Rect *window_dest) {
    SDL_RenderCopy(m_renderer, m_texture, texture_src, window_dest); // Desenha no buffer traseiro
}
